...
def process_saved_data_improved(data_folder="lidar_data"):
    ...
    for pkl_file in sorted(pkl_files):
        ...
        try:
            ...
            for frame_data in point_clouds_data:
                ...
                # Filtrar puntos que estén muy cerca de los demás (por ejemplo, puntos con una distancia pequeña)
    ...
    # Ahora realiza la segmentación sobre todos los puntos combinados
    planes = segmenter.segment_planes_improved(all_points, max_planes=5)

    if planes:
        frames_with_planes += 1
        total_planes_detected += len(planes)
        print(f"  ✓ Detectados {len(planes)} planos!")
        
        # Aquí puedes proceder a la identificación de paredes o cualquier otra operación
        identify_walls(planes, lidar_position=np.array([0, 0, 2.5]))  # Ajusta según la posición del LiDAR
        # Mostrar info de cada plano (opcional)
        for i, plane in enumerate(planes):
            normal = plane['normal']
            angle_deg = np.degrees(np.arccos(np.abs(normal[2])))
            print(f"    Plano {i+1}: {plane['size']} puntos, ángulo: {angle_deg:.1f}°")
        
        # Visualizar si hay planos interesantes
        if any(plane['size'] > 20 for plane in planes):
            ...
            visualize_lidar_points(original_cloud)
            
            # Visualizar los planos con las cajas delimitadoras
